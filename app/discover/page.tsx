...
export default function DiscoverPage() {
  ...
  const filteredDJs = useMemo(() => {
    ...
    if (!q) return djs;
    return djs.filter((dj) => [dj.dj_name, dj.bio || "", dj.location || "", ...(dj.genres || [])].join(" ").toLowerCase().includes(q));
  }, [djs, search]);

  const filteredGigs = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return gigs;
    return gigs.filter((gig) => [gig.title, gig.description || "", gig.location, ...(gig.genres || [])].join(" ").toLowerCase().includes(q));
  }, [gigs, search]);
