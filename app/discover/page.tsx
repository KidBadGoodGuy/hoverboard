...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setRole(user?.role === "client" ? "client" : "dj");

      const [djResult, gigResult] = await Promise.all([
        supabase.from("dj_profiles").select("user_id, dj_name, bio, location, genres, price, profile_photo").order("dj_name"),
        supabase.from("gigs").select("id, title, description, event_date, start_time, end_time, location, budget_min, budget_max, genres, status").eq("status", "open").order("event_date")
      ]);

      if (djResult.error) setMessage(djResult.error.message);
      else setDjs((djResult.data as DJ[]) || []);
      if (gigResult.error) setMessage(gigResult.error.message);
      else setGigs((gigResult.data as Gig[]) || []);
      setLoading(false);
    }
    void load();
  }, [router]);
  ...
  const genres = useMemo(() => Array.from(new Set((mode === "djs" ? djs.flatMap((d) => d.genres || []) : gigs.flatMap((g) => g.genres || [])))).sort(), [mode, djs, gigs]);
  ...
  return (
    <main className="dashboard-page">
      ...
      <section className="dashboard-content">
        <div className="hero-small"><p className="eyebrow">DISCOVER</p><h1>Find DJs and gigs.</h1><p>Search the HOVERBOARD marketplace from either account.</p></div>

        <section className="card">
          <div className="actions" style={{ marginBottom: "18px" }}>
            <button type="button" className={mode === "djs" ? "button primary" : "button"} onClick={() => { setMode("djs"); clearFilters(); }}>Find DJs</button>
            <button type="button" className={mode === "gigs" ? "button primary" : "button"} onClick={() => { setMode("gigs"); clearFilters(); }}>Find gigs</button>
          </div>
          <label>Search<input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={mode === "djs" ? "DJ name, genre, or location" : "Gig title, genre, or location"} /></label>
